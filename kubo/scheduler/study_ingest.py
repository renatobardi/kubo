...
# Mesmos pinos do routes/study.py (gate humano no cron, não no catálogo).
...
def execute_study_ingest_job(db: Any, *, tenant_id: RecordID, user_id: RecordID) -> None:
    ...
    try:
        ...
        _log.warning("study.ingest.summarizer_setup_failed", exc_info=True)
    ...
    try:
        ...
        _log.warning("study.ingest.sectionizer_setup_failed", exc_info=True)
    ...
    for material in pending:
        try:
            ...
            failed += 1
            _log.warning("study.ingest.parse_failed", material=str(material.id), exc_info=True)
            _mark_failed(
                db,
                tenant_id=tenant_id,
                user_id=user_id,
            ...
            failed += 1
            _log.warning("study.ingest.llm_failed", material=str(material.id), exc_info=True)
            _mark_failed(
                db,
                tenant_id=tenant_id,
                user_id=user_id,
            ...
            failed += 1
            _log.exception("study.ingest.unexpected", material=str(material.id))
            _mark_failed(
                db,
                tenant_id=tenant_id,
                user_id=user_id,
