...
def _fetch_releases(base_url: str, token: str, owner: str, repo: str) -> list[dict[str, Any]]:
    ...
    do catálogo (não é input hostil, ao contrário do `feed_url` de terceiro em `feed.py`) —
    não há redirect legítimo a seguir aqui, então desligar fecha por construção a classe de
    SSRF-via-redirect que `feed.py` precisa de um guard dedicado pra mitigar. Streaming +
    teto de bytes (mesmo padrão de `feed.py._fetch`) evita bufferizar uma resposta gigante
    inteira antes do `_CONTENT_CAP` cortar qualquer coisa."""
    ...
    try:
        with httpx.Client(timeout=_TIMEOUT, follow_redirects=False) as client:
            with client.stream(
                "GET", url, headers=_headers(token), params={"per_page": _PER_PAGE}
            ) as resp:
...
def _release_to_payload(
    ...
    return ItemPayload(
        ...
        # tag_name passa pela MESMA limpeza que content/title (achado do security-reviewer):
        # gravado cru, um surrogate solto sobreviveria até o encoder CBOR estrito do SDK
        # SurrealDB e abortaria a persistência do BATCH inteiro (sem try/except por payload
        # em run_worker) — o oposto da falha-parcial que este worker promete no docstring.
        metadata={
