...
- **Consulta de advisor #2 (conclusão) TAMBÉM rodou DEGRADADA**, compensada com verificação empírica contra o código real. Veredito: **GO condicionado a 3 correções** — todas aplicadas antes do PR: **F1** (crítica) `revalidate_instances="always"` nos modelos do contrato (o default `never` deixava a "validação antes de persistir" furável por `model_construct` — a claim central do ADR era refutável pelo código); **F2** `ResolvedIntegration.secret` = `field(repr=False)`; **F3** `ErrorInfo.message` = `Field(max_length=500)`. Menores aplicados no mesmo passo: Protocol `RunContext.integrations` → `Mapping[str, Any]` (alinha com o concreto), `_build_context(manifest: WorkerManifest)`, `kind="contract"` p/ RunResult inválido. ADR-0009 emendado (item I + item VIII).
- **Revisão de segurança (`security-reviewer`) rodada antes do PR** (política CLAUDE.md p/ contracts/runtime/catalogs). Achados críticos independentes convergiram com o advisor: **(#1)** vazamento de segredo por `repr` de `ResolvedIntegration`/`RunContext` → fechado por `field(repr=False)`; **(#2)** `str(ValidationError)` embute input + `IntegrationAuth` ecoava o `secret_ref` candidato → mensagens sanitizadas (`errors(include_input=False)`, sem ecoar valor); **(#3)** `_persist` fora do try (run travava em `running`) → movido para dentro da fronteira. Todos com teste de regressão (6 testes de segurança novos, RED→GREEN). Sólidos sem achado: binds da store, `yaml.safe_load`, least-privilege, TOCTOU, `_resolve_secret`.
- **Desvio de orquestração declarado (rule 6, transparência):** o GREEN de `contracts/`/`runtime/` foi escrito NA THREAD PRINCIPAL (Opus), não delegado ao `implementer` (Sonnet) como o plano 4.1.2/4.2 previa. Justificativa: a sessão inteira é fronteira de segurança (rule 1: paths onde a thread valida linha a linha; delegar a frontier toda = briefing ≈ código). O `test-writer` (Sonnet) produziu o RED do contrato (M4.1). Aceitável para esta sessão (advisor concordou), **não deve virar hábito** — se o feed do M5 nascer na thread, é erosão de política. Lição do checkpoint: o que a fronteira precisava não era mais modelo no GREEN, era revisão adversarial com verificação empírica (F1 passou mesmo com Opus escrevendo e validando; só a revisão pegou).
- **Pendências nomeadas para o M5:** (a) aresta `item→run` (carry-over do 0003, não implementada aqui); (b) deny completo de least-privilege — já implementado (não foi cortado); (c) `rss.yaml` — já entregue (não foi cortado); (d) `load_integrations`: nome duplicado entre YAMLs é last-wins silencioso e `name:` pode divergir do filename — validar quando houver 2+ integrações; (e) `RunContext.integrations` no Protocol é `Mapping[str, Any]` (loose) — o feed do M5 fará narrowing para `ResolvedIntegration`.
