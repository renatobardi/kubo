...
- **D45 (REVERTIDA em 2026-07-15):** conta-máquina adiada pro futuro — **o agente usa o PAT fine-grained DO DONO** (na própria conta), restrito ao repo kubo. Consequência (o advisor já previu e abençoou com condição): PR de agente NÃO é distinguível por autor (é o dono) → **identificação por PREFIXO DE BRANCH** (ex.: `agent/*`, derivado do flow id); **require-review DESLIGADO** (o dono não aprova o próprio PR) → o gate humano vira **disciplina do dono, não construção do GitHub**, e o **ADR-0021 nomeia isso explicitamente como postura de segurança MAIS FRACA** (com gatilho: virou fricção/risco → cria a conta-máquina kubo-dev e restaura o require-review). Path-guard e PAT-sem-Workflows (E2/E3) continuam valendo — a proteção estrutural principal permanece.

## Emendas do advisor (GO condicionado — todas incorporadas)

### Segurança do agente no repo principal (0018b)
- **E1 — (REVISADA, D45 revertida): PAT fine-grained DO DONO**, restrito ao repo kubo, contents+PR write. Identificação do PR de agente **por prefixo de branch** (`agent/*`, derivado do flow id — o worker já deriva o nome do branch do flow id), **não** por autor (o autor é o dono). Require-review desligado; gate = disciplina do dono (ver D45 revertida).
- **E2 — PAT SEM a permissão Workflows** (a proteção estrutural mais importante): o GitHub **rejeita** push que toque `.github/workflows/` — o CI não pode ser neutralizado pelo próprio PR. Sem isso, path-guard é teatro. **VALE INTEGRALMENTE mesmo com PAT do dono.**
- **E3 — Path-guard como job de CI + required status check:** se o **branch casa `agent/*`** (não mais `github.actor`), o diff deve estar contido em `kubo/workers/**`, `tests/workers/**`, `catalogs/**`. Lógica **inline no workflow** (nunca script do repo que o PR poderia editar). Fora do allowlist → merge bloqueado por branch protection. **É a proteção estrutural que substitui o require-review perdido — logo é INCORTÁVEL nesta configuração.**
