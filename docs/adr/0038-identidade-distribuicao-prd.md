...
- **Provedor: conta SMTP da Hostinger, não Resend.** A Hostinger (onde `oute.pro` já vive) provê SMTP autenticado; o dono configurou e **validou conexão/autenticação SMTP** no `.env` do kubo-prd — o teste prova config e relay, **não entrega em inbox** (isso depende de SPF/DKIM/DMARC + envio real observado, pendente na Fatia D). Resend fica reavaliável no futuro. O `EmailDigestWorker`/`SmtpConfig` (ADR-0031) **não muda** — relay SMTP por env, exatamente o cenário de zero-reescrita que o §I previa.
