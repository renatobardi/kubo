...
**Decisão:** deletar o último Material em `planning` **auto-reverte o Tema a `draft`**. O estado `planning` sem Materiais não tem sentido — o Plano é proposição sobre capítulos de Materiais. O auto-revert previne o estado em vez de tratar o crash depois. A rota `delete_material` checa `count_materials_by_topic` após o delete; se 0 e o estado era `planning`, chama `set_topic_state("draft")` e redireciona com `?notice=voltou-rascunho` (banner informativo no template).
